...
const ROUTING_KEYS = ["execution.finished", "execution.failed"];

type ExecutionEventPayload = { serviceOrderId?: string };

const PERMANENT_FAILURE = /not found|Unknown execution event|Missing serviceOrderId/i;

export class RabbitMQExecutionEventConsumer {
  ...
  private async handleMessage(message: ConsumeMessage): Promise<void> {
    ...
    try {
      const payload = JSON.parse(message.content.toString()) as ExecutionEventPayload;
      if (!payload.serviceOrderId) {
        throw new Error(`Missing serviceOrderId in ${routingKey} event`);
      }

      await this.serviceOrderController.applyExecutionEvent(payload.serviceOrderId, routingKey);
      channel.ack(message);
    } catch (error) {
