...
O `os-service` e o `fiap-soat-billing-service` se comunicam via **RabbitMQ** em ambas as direções (o os-service publica `quotation.requested`; o billing publica `payment.approved`/`payment.failed`, consumidos também pelo `execution-service`) e via REST apenas para `quotation.rejected` (o billing chama `OS_SERVICE_URL` para atualizar o status da OS). O broker RabbitMQ sobe junto com o `docker-compose.dev.yml` deste repositório (`os-service`) e é compartilhado pelos demais serviços. Como cada repositório tem seu próprio `docker-compose`, subir os serviços separadamente cria stacks isoladas em redes Docker diferentes — os containers não conseguem se enxergar por padrão.
